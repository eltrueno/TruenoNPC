...
public class TruenoNPC_v1_9_r1 implements TruenoNPC {
    ...
    private static List<TruenoNPC_v1_9_r1> npcs = new ArrayList<TruenoNPC_v1_9_r1>();
    ...
    public static void startTask(Plugin plugin){
        if(!taskstarted){
            taskstarted = true;
            TruenoNPC_v1_9_r1.plugin = plugin;
            Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
                @Override
                public void run() {
                    for(TruenoNPC_v1_9_r1 nmsnpc : npcs){
    ...
    public TruenoNPC_v1_9_r1(Location location, TruenoNPCSkin skin){
        npcid = id++;
        this.skin = skin;
    ...
    private void spawnEnttity(Player p){
        ...
        Bukkit.getScheduler().runTaskLater(TruenoNPC_v1_9_r1.plugin, new Runnable(){
            @Override
            public void run() {
                rmvFromTablist(p);
            }
        ...
        try{
            if(this.skin.getSkinType()==SkinType.PLAYER){
                if(this.player_entity.get(p)!=null){
                    profile = this.player_entity.get(p).getProfile();
            ...
            if(this.player_entity.get(p)!=null){
                entityID = this.player_entity.get(p).getId();
            }

            PacketPlayOutScoreboardTeam removescbpacket = new PacketPlayOutScoreboardTeam();
            setValue(removescbpacket,"a", profile.getName());
    ...
    private void addToTablist(Player p){
        ...
        List<PacketPlayOutPlayerInfo.PlayerInfoData> players = Lists.newArrayList();
        players.add(data);
